...
void ColladaTransfer::BuildIncrementalMesh (const TopoDS_Shape&  Shape,
					const Standard_Real  Deflection,
					const Handle(ColladaMesh_Mesh)& Mesh)
{
  if (Deflection <= Precision::Confusion ()) {
    Standard_ConstructionError::Raise ("ColladaTransfer::BuildIncrementalMesh");
    }
  ...
  for (TopExp_Explorer itf(Shape,TopAbs_FACE); itf.More(); itf.Next()) {
    ...
    try {
      OCC_CATCH_SIGNALS
      #ifdef DEB
      cout << "theTriangles.Length" << theTriangles.Length() << "\n";
      #endif
      for (i=1;i<=theTriangles.Length();i++) {
	      Standard_Integer V1=0, V2=0, V3=0;
	      ...
	      // compute normals
          gp_Vec normal1 = NORMAL(V1);
          gp_Vec normal2 = NORMAL(V2);
          gp_Vec normal3 = NORMAL(V3);
          gp_Dir d1 = normal1.Transformed(Loc.Transformation());
          gp_Dir d2 = normal2.Transformed(Loc.Transformation());
          gp_Dir d3 = normal3.Transformed(Loc.Transformation());
        /*********/
	    //check angle between vectors
	      gp_Vec V1V2(P1, P2), V2V3(P2, P3);
	      Standard_Integer A=0,B=0,C=0;
          Standard_Real n1x=0.,n1y=0.,n1z=0.,n2x=0.,n2y=0.,n2z=0.,n3x=0.,n3y=0.,n3z=0.;
	    ...
	    //	vec.Transform(loc);
	      if (modul1>Precision::Confusion () && modul2>Precision::Confusion ()) {
	          ...
	          if ( an > PI/2)  {
	              A=V3;B=V2;C=V1;
                  n1x = d3.X();n1y = d3.Y();n1z = d3.Z();
                  n2x = d2.X();n2y = d2.Y();n2z = d2.Z();
                  n3x = d1.X();n3y = d1.Y();n3z = d1.Z();
	            }
	          else {
	              A=V1;B=V2;C=V3;
	              n1x = d1.X();n1y = d1.Y();n1z = d1.Z();
                  n2x = d2.X();n2y = d2.Y();n2z = d2.Z();
                  n3x = d3.X();n3y = d3.Y();n3z = d3.Z();
	            }
	          }
    Mesh->AddTriangle (A, B, C, n1x,n1y,n1z, n2x,n2y,n2z,n3x,n3y,n3z);
      }
    }
    catch(Standard_Failure)
      {
#ifdef DEB
	cout << "Fail in ColladaTransfer::BuildIncrementalMesh"   << endl;
#endif
      }
  }
}
